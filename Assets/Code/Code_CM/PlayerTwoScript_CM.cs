...
public class PlayerTwoScript_CM : MonoBehaviour
{
    ...
    public int extraSpeed; //extra speed for walking
    ...
    public static PlayerTwoScript_CM Instance { get; private set; }
    ...
    {
        //Laufen & Springen Code
        inWalk = playerInput.TestMovement.walk.ReadValue<Vector2>();

        //Springen Code
        inJump = playerInput.TestMovement.jump.ReadValue<float>();

        //Vertikal Schießen Code
        inShootV = playerInput.TestMovement.vShoot.ReadValue<float>();

        //Horizontal Schießen Code
        inShootH = playerInput.TestMovement.hShoot.ReadValue<float>();
    }
    ...
    private void Walk()
    {
         rbP.velocity = new Vector2(inWalk.x * speed * extraSpeed * Time.deltaTime, rbP.velocity.y);
    }
